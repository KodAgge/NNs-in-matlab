...
% Computing gradients
ComputeGradients(trainX, trainY, trainy, W, b, lambda);
[grad_W, grad_b] = ComputeGradients(trainX, trainY, trainy, W, b, lambda);
[ngrad_b, ngrad_W] = ComputeGradsNumSlow(trainX, trainY, ...
    trainy, W, b, lambda, 1e-6);
...
function [J, l_svm] = ComputeCost(X, Y, y, W, b, lambda)
    penalizer = lambda * sum(W.^2, 'all');
    l_svm = ComputeLoss(X, Y, y, W, b);
    [~, n] = size(X);
    J = l_svm / n + penalizer;
end

function [L, margins] = ComputeLoss(X, Y, y, W, b)
    s = W * X + b;
    S = s - ones(size(s,1)) * (s .* Y) + 1 - Y;
    margins = max(0, S);
    L = sum(margins, 'all');
end
...
function [grad_W, grad_b] = ComputeGradients(X, Y, y, W, b, lambda)
    [~, margins] = ComputeLoss(X, Y, y, W, b);
    G = margins;
    G(margins > 0) = 1; % The active elements j =/= y
    G = G - Y.*sum(G); % Sum of active elements added for j == y
    [~, n] = size(X);
    L_W = G * X.' / n;
    grad_b = G * ones([n, 1]) / n;
...
function [grad_b, grad_W] = ComputeGradsNumSlow(X, Y, y, W, b, lambda, h)
    ...
    for i=1:length(b)
        b_try = b;
        b_try(i) = b_try(i) - h;
        c1 = ComputeCost(X, Y, y, W, b_try, lambda);
        b_try = b;
        b_try(i) = b_try(i) + h;
        c2 = ComputeCost(X, Y, y, W, b_try, lambda);
        grad_b(i) = (c2-c1) / (2*h);
    end
    ...
    for i=1:numel(W)
        ...
        W_try = W;
        W_try(i) = W_try(i) - h;
        c1 = ComputeCost(X, Y, y, W_try, b, lambda);

        W_try = W;
        W_try(i) = W_try(i) + h;
        c2 = ComputeCost(X, Y, y, W_try, b, lambda);
...
function [grad_b, grad_W] = ComputeGradsNum(X, Y, y, W, b, lambda, h)
    ...
    c = ComputeCost(X, Y, y, W, b, lambda);
...
function [Wstar, bstar] = MiniBatchGD(X, Y, y, valX, valY, valy, GDparams, W, b, lambda)
    % Extract parameters
    n_batch = GDparams(1);
    eta = GDparams(2);
    n_epochs = GDparams(3);
    ...
    for i=1:n_epochs
        ...
        % Generate random order for batches
        batch_order = randperm(n/n_batch);
        ...
        for j=batch_order
            ...
            % Compute gradients
            [grad_W, grad_b] = ComputeGradients(X_batch, Y_batch, y_batch, W, b, lambda);
        ...
        [J, L] = ComputeCost(valX, valY, valy, W, b, lambda);
        J_val = [J_val, J];
        L_val = [L_val, L];
        
        [J, L] = ComputeCost(X, Y, y, W, b, lambda);
        J_train = [J_train, J];
        L_train = [L_train, L];
